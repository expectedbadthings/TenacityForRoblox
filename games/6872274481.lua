-- Tenacity native port of the BedWars game module.
-- Original game logic retained; modules register through the Tenacity API.
...
local vape = tenacity
-- R10 keeps these legacy core fields compatible, while module registration below uses the public Tenacity API.
if not vape.guiscale and vape.gui and vape.gui:FindFirstChild('ScaledGui') then
	vape.guiscale = vape.gui.ScaledGui:FindFirstChildOfClass('UIScale') or {Scale = 1}
end
...
local getcustomasset = vape.Libraries.gettenacityasset or vape.Libraries.getcustomasset
...
local function addBlur(parent)
	...
	blur.Image = getcustomasset('tenacity/assets/new/blur.png')
...
run(function()
	...
	for i, v in remoteNames do
		...
		if remote == '' then
			notif('Tenacity', 'Failed to grab remote ('..i..')', 10, 'alert')
		end
		remotes[i] = remote
	end
...
run(function()
	...
	Velocity = tenacity:Module('Combat', {
		Name = 'Velocity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastBreak = tenacity:Module('Movement', {
		Name = 'FastBreak',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	NoFall = tenacity:Module('Movement', {
		Name = 'NoFall',
		Function = function(callback)
			if callback then
				local tracked = 0
...
run(function()
	...
	local ProjectileAimbot = tenacity:Module('Movement', {
...
run(function()
	...
	ProjectileAura = tenacity:Module('Movement', {
		Name = 'ProjectileAura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	StorageESP = tenacity:Module('Render', {
		Name = 'StorageESP',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBalloon = tenacity:Module('Misc', {
		Name = 'AutoBalloon',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoPearl = tenacity:Module('Misc', {
		Name = 'AutoPearl',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	AutoToxic = tenacity:Module('Misc', {
		Name = 'AutoToxic',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoVoidDrop = tenacity:Module('Misc', {
		Name = 'AutoVoidDrop',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MissileTP = tenacity:Module('Misc', {
		Name = 'MissileTP',
		Function = function(callback)
			if callback then
				MissileTP:Toggle()
...
run(function()
	...
	ShopTierBypass = tenacity:Module('Misc', {
		Name = 'ShopTierBypass',
		Function = function(callback)
			if callback then
...
run(function()
	TrapDisabler = tenacity:Module('Misc', {
		Name = 'TrapDisabler',
...
run(function()
	...
	AutoSuffocate = tenacity:Module('Exploit', {
		Name = 'AutoSuffocate',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AutoTool = tenacity:Module('Exploit', {
		Name = 'AutoTool',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedPlates = tenacity:Module('Exploit', {
		Name = 'BedPlates',
		Function = function(callback)
			if callback then
...
run(function()
	...
	BedProtector = tenacity:Module('Exploit', {
		Name = 'BedProtector',
		Function = function(callback)
			if callback then
...
run(function()
			...
			local mounted = bedwars.Roact.mount(create('BillboardGui', {
				...
				create('Frame', {
					...
					create('ImageLabel', {
						...
						Image = getcustomasset('tenacity/assets/new/blur.png'),
	...
	Breaker = tenacity:Module('Exploit', {
		Name = 'Breaker',
		Function = function(callback)
			if callback then
				for _ = 1, 30 do
...
run(function()
	...
	ChestSteal = tenacity:Module('Exploit', {
		Name = 'ChestSteal',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Schematica = tenacity:Module('Exploit', {
		Name = 'Schematica',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ArmorSwitch = tenacity:Module('Player', {
		Name = 'ArmorSwitch',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBank = tenacity:Module('Player', {
		Name = 'AutoBank',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AutoBuy = tenacity:Module('Player', {
		Name = 'AutoBuy',
		Function = function(callback)
			if callback then
				repeat task.wait() until store.queueType ~= 'bedwars_test'
...
run(function()
	...
	AutoConsume = tenacity:Module('Player', {
		Name = 'AutoConsume',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local function CreateWindow(self)
		...
		close.Image = getcustomasset('tenacity/assets/new/close.png')
		...
		searchicon.Image = getcustomasset('tenacity/assets/new/search.png')
	...
	vape.Components.HotbarList = function(optionsettings, children, api)
		...
		textbuttonicon.Image = getcustomasset('tenacity/assets/new/add.png')
		...
		function optionapi:AddHotbar(data)
			...
			close.BackgroundColor3 = Color3.new(1, 1, 1)
			close.BackgroundTransparency = 1
			close.Image = getcustomasset('tenacity/assets/new/closemini.png')
			close.ImageColor3 = color.Light(uipallet.Text, 0.2)
			close.ImageTransparency = 0.5
			close.AutoButtonColor = false
			close.Parent = hotbar
	...
	AutoHotbar = tenacity:Module('Player', {
		Name = 'AutoHotbar',
		Function = function(callback)
			if callback then
...
run(function()
	...
	local FastConsume = tenacity:Module('Player', {
		Name = 'FastConsume',
		Function = function(callback)
			if callback then
...
run(function()
	...
	FastDrop = tenacity:Module('Player', {
		Name = 'FastDrop',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	BedBreakEffect = tenacity:Module('Render', {
...
run(function()
	...
	local Crosshair = tenacity:Module('Render', {
		Name = 'Crosshair',
		Function = function(callback)
			if callback then
...
run(function()
	...
	DamageIndicator = tenacity:Module('Render', {
...
run(function()
	...
	FPSBoost = tenacity:Module('Render', {
		Name = 'FPS Boost',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitColor = tenacity:Module('Render', {
		Name = 'Hit Color',
		Function = function(callback)
			if callback then 
				repeat
...
run(function()
	...
	Interface = tenacity:Module('Render', {
		Name = 'Interface',
		Function = function(callback)
...
run(function()
	...
	KillEffect = tenacity:Module('Render', {
		Name = 'Kill Effect',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ReachDisplay = tenacity:Module('Render', {
		Name = 'Reach Display',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SoundChanger = tenacity:Module('Render', {
		Name = 'SoundChanger',
		Function = function(callback)
			if callback then
...
run(function()
	...
	UICleanup = tenacity:Module('Render', {
		Name = 'UI Cleanup',
		Function = function(callback)
			for i, v in (callback and new or old) do
				for i2, v2 in v do
...
run(function()
	...
	Viewmodel = tenacity:Module('Render', {
		Name = 'Viewmodel',
		Function = function(callback)
...
run(function()
	...
	WinEffect = tenacity:Module('Render', {
		Name = 'WinEffect',
		Function = function(callback)
			if callback then
...
-- Added Tenacity-native BedWars modules: Speed +, Fly +, TexturePacks, InfiniteJump.
...
run(function()
	...
	local function setWindWalkerBoost(enabled)
		pcall(function()
			local controller = bedwars.WindWalkerController
			local updateSpeed = controller and controller.updateSpeed
			if not updateSpeed or not debug.getconstants or not debug.setconstant then return end

			if not windWalkerIndex then
				for index, value in ipairs(debug.getconstants(updateSpeed)) do
					if type(value) == 'number' and value > 1 and value < 1.3 then
						windWalkerIndex = index
						windWalkerOriginal = value
						break
					end
				end
			end

			if windWalkerIndex and windWalkerOriginal then
				debug.setconstant(updateSpeed, windWalkerIndex, enabled and 1.45 or windWalkerOriginal)
			end
		end)
	end

	BetterSpeed = tenacity:Module('Movement', {
		Name = 'Speed +',
		Function = function(callback)
			...
			if callback then
				...
				BetterSpeed:Clean(runService.PreSimulation:Connect(function(dt)
					...
					if (Fly and Fly.Enabled) or (BetterFly and BetterFly.Enabled) or InfiniteFly.Enabled or (LongJump and LongJump.Enabled) then return end
	...
	Pulse = BetterSpeed:Setting({
		Type = 'toggle',
		Name = 'Pulse',
		Default = true
	})
	PulseStrength = BetterSpeed:Setting({
		Type = 'slider',
		Name = 'Pulse Strength',
		Min = 1,
		Max = 1.5,
	...
	PulseDelay = BetterSpeed:Setting({
		Type = 'slider',
		Name = 'Pulse Delay',
		Min = 0.15,
		Max = 0.5,
	...
	WallCheck = BetterSpeed:Setting({
		Type = 'toggle',
		Name = 'Wall Check',
		Default = true
	})
	AutoJump = BetterSpeed:Setting({
		Type = 'toggle',
		Name = 'AutoJump',
		Function = function(enabled)
			if AlwaysJump and AlwaysJump.Object then
				AlwaysJump.Object.Visible = enabled
			end
		end
	})
	AlwaysJump = BetterSpeed:Setting({
		Type = 'toggle',
		Name = 'Always Jump',
		Visible = false,
		Darker = true
...
run(function()
	...
	BetterFly = tenacity:Module('Movement', {
		Name = 'Fly +',
		Function = function(callback)
			...
			if callback then
				...
				BetterFly:Clean(runService.PreSimulation:Connect(function(dt)
					...
					local targetVel = (moveDir * Speed.Value) + Vector3.new(0, (up + down) * Vertical.Value, 0)
					...
					local filters = {lplr.Character, gameCamera}
					if AntiFallPart then
						table.insert(filters, AntiFallPart)
					end
					...
					if TPDown.Enabled and not InfiniteFly.Enabled then
						...
						if airTime > 2.1 and not tpActive then
							...
							if ray then
								...
								root.CFrame = CFrame.new(root.Position.X, ray.Position.Y + humanoid.HipHeight, root.Position.Z) * rotation
							end
						end
						...
						if tpActive then
							...
							if tick() >= tpUntil then
								local rotation = root.CFrame.Rotation
								root.CFrame = CFrame.new(root.Position.X, tpRestoreY, root.Position.Z) * rotation
								tpActive = false
								tpRestoreY = nil
							end
						end
	...
	Speed = BetterFly:Setting({Type = 'slider', Name = 'Speed', Min = 1, Max = 23, Default = 23, Suffix = 'studs'})
	Vertical = BetterFly:Setting({Type = 'slider', Name = 'Vertical Speed', Min = 1, Max = 100, Default = 60, Suffix = 'studs'})
	Accel = BetterFly:Setting({Type = 'slider', Name = 'Acceleration', Min = 1, Max = 25, Default = 14})
	Hover = BetterFly:Setting({Type = 'toggle', Name = 'Hover', Default = true})
	WallCheck = BetterFly:Setting({Type = 'toggle', Name = 'Wall Check', Default = true})
	TPDown = BetterFly:Setting({Type = 'toggle', Name = 'TP Down', Default = true})
end)
...
run(function()
	...
	local function clearVisuals()
		...
		for _, model in addedModels do
			if model and model.Parent then
				model:Destroy()
			end
		end
		table.clear(hiddenParts)
		table.clear(addedModels)
	end

	local function destroyImport()
		if importedPack then
			importedPack:Destroy()
			importedPack = nil
		end
		table.clear(packIndex)
	end

	local function hideOriginalParts(container)
		for _, object in container:GetDescendants() do
			if object:IsA('BasePart') then
				if hiddenParts[object] == nil then
					hiddenParts[object] = object.Transparency
				end
	...
	local function attachModel(container, source, offset, thirdPerson)
		local handle = container:FindFirstChild('Handle') or container:WaitForChild('Handle', 2)
		...
		elseif model:IsA('Model') then
			weldPart = model.PrimaryPart or model:FindFirstChildWhichIsA('BasePart', true)
		end
		...
		local targetCFrame = handle.CFrame * offset * CFrame.Angles(0, math.rad(-50), 0)
		if thirdPerson then
			targetCFrame *= CFrame.new(0.6, 0, -0.9)
		end
	...
	local function applyToTool(tool)
		...
		for _, entry in packIndex do
			if entry.name == tool.Name then
				hideOriginalParts(tool)
				attachModel(tool, entry.model, entry.offset, false)

				local character = lplr.Character
				local thirdPersonTool = character and character:FindFirstChild(tool.Name)
				if thirdPersonTool then
					hideOriginalParts(thirdPersonTool)
					attachModel(thirdPersonTool, entry.model, entry.offset, true)
				end
				break
			end
		end
	...
	local function buildIndex(import)
		local function add(name, objectName, offset)
			local model = import:FindFirstChild(objectName)
			if model then
				table.insert(packIndex, {name = name, offset = offset, model = model})
			end
		end

		add('wood_sword', 'Wood_Sword', CFrame.Angles(0, math.rad(-89), math.rad(-90)))
		add('stone_sword', 'Stone_Sword', CFrame.Angles(0, math.rad(-89), math.rad(-90)))
		add('iron_sword', 'Iron_Sword', CFrame.Angles(0, math.rad(-89), math.rad(-90)))
		add('diamond_sword', 'Diamond_Sword', CFrame.Angles(0, math.rad(-89), math.rad(-90)))
		add('emerald_sword', 'Emerald_Sword', CFrame.Angles(0, math.rad(-89), math.rad(-90)))

		for _, material in {'Wood', 'Diamond', 'Emerald', 'Stone', 'Iron', 'Gold'} do
			add(material:lower()..'_pickaxe', material..'_Pickaxe', CFrame.Angles(0, math.rad(-180), math.rad(-95)))
			add(material:lower(), material, CFrame.Angles(0, math.rad(-90), math.rad(table.find({'Emerald', 'Diamond'}, material) and 90 or -90)))
		end
	end
	...
	local function loadPack()
		clearVisuals()
		destroyImport()
		local assetId = pack1 and packassetids[pack1.Value]
		if not assetId then return false end

		local success, result = pcall(game.GetObjects, game, assetId)
		if not success or not result or not result[1] then
			tenacity:Notify('TexturePacks', 'Failed to load '..tostring(pack1.Value), 5, 'alert')
			return false
		end

		importedPack = result[1]
		importedPack.Name = 'TenacityTexturePack'
		importedPack.Parent = replicatedStorage
		buildIndex(importedPack)

		local viewmodel = gameCamera:FindFirstChild('Viewmodel')
		if viewmodel then
			for _, tool in viewmodel:GetChildren() do
				applyToTool(tool)
			end
		end
		return true
	...
	TexturePacks = tenacity:Module('Render', {
		Name = 'TexturePacks',
		Tooltip = 'Gives tools custom BedWars textures.',
		Function = function(callback)
			if callback then
				loadPack()
				local viewmodel = gameCamera:FindFirstChild('Viewmodel') or gameCamera:WaitForChild('Viewmodel', 5)
				if viewmodel then
					TexturePacks:Clean(viewmodel.ChildAdded:Connect(function(tool)
						task.defer(applyToTool, tool)
					end))
				end
			else
				clearVisuals()
	...
	local list = {}
	for name in packassetids do
		table.insert(list, name)
	end
	table.sort(list)
	pack1 = TexturePacks:Setting({
		Type = 'dropdown',
		Name = 'Pack',
		List = list,
		Default = 'Exhibition',
...
run(function()
	local InfiniteJump
	local Velocity
	local keyboardJumping = false
	local touchJumping = false

	InfiniteJump = tenacity:Module('Misc', {
		Name = 'InfiniteJump',
		Function = function(callback)
			if callback then
				InfiniteJump:Clean(inputService.InputBegan:Connect(function(input, gameProcessed)
					...
					if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == Enum.KeyCode.Space then
						keyboardJumping = true
					end
				end))
				InfiniteJump:Clean(inputService.InputEnded:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode == Enum.KeyCode.Space then
						keyboardJumping = false
					end
				end))

				if inputService.TouchEnabled then
					local touchGui = lplr.PlayerGui:FindFirstChild('TouchGui') or lplr.PlayerGui:WaitForChild('TouchGui', 5)
				...
				InfiniteJump:Clean(runService.RenderStepped:Connect(function()
					if not (keyboardJumping or touchJumping) or not entitylib.isAlive then return end
					local root = entitylib.character.RootPart or (lplr.Character and lplr.Character.PrimaryPart)
					if root then
						local current = root.AssemblyLinearVelocity
						root.AssemblyLinearVelocity = Vector3.new(current.X, Velocity.Value, current.Z)
					end
				end))
			else
				keyboardJumping = false
		...
		Tooltip = 'Allows infinite jumping while jump is held.'
	})
	Velocity = InfiniteJump:Setting({
		Type = 'slider',
		Name = 'Velocity',
		Min = 50,
		Max = 300,
