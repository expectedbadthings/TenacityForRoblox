...
run(function()
	...
	local function track(parent, title, minimum, maximum, get, set, y, decimals, style)
		...
		value.FocusLost:Connect(function()
			...
			if number and math.isfinite(number) then set(math.clamp(number,minimum,maximum),true) end
		end)
		watch(rail,function()
	...
	local function toggleControl(parent,name,get,set,style)
		...
		watch(switch,function()
			...
			if knob:GetAttribute('Enabled')~=enabled then
				knob:SetAttribute('Enabled',enabled)
				tween:Tween(knob,uiMotion,{Position=modern and UDim2.fromOffset(enabled and 20 or 0,0) or UDim2.fromOffset(enabled and 21 or 3,2)})
			end
	...
	function ui:DrawControls(parent, owner, includeBind, style)
		...
		for index, option in controls do
			...
			elseif kind == 'Dropdown' then
				...
				local labelX=modern and 26 or 10
				row.Size=UDim2.new(1,0,0,baseHeight)
				label(row,name,labelX,0,modern and 92 or compact and 135 or 188,modern and 44 or compact and 32 or 22,modern and 12 or compact and 12 or 14)
				local selection=button(row,'',0,modern and 8 or compact and 5 or 24,modern and 104 or compact and 135 or 180,function()
					open=not open
					if open and choices then choices:Destroy(); choices=nil end
					tween:Tween(row,uiMotion,{Size=UDim2.new(1,0,0,baseHeight)})
					if open then
						local values={}
						for _,value in props.List or {option.Value} do if value~=option.Value then table.insert(values,value) end end
						local choiceHeight=modern and 30 or compact and 24 or 30
						local height=math.min(#values,7)*choiceHeight
						local listX=modern and 0 or compact and 0 or 10
						local listY=baseHeight
						choices=scroll(row,listX,listY,modern and 104 or compact and 135 or 180,height)
						if modern then choices.Position=UDim2.new(1,-128,0,listY); choices.Size=UDim2.fromOffset(104,height)
						elseif compact then choices.Position=UDim2.new(1,-145,0,listY); choices.Size=UDim2.fromOffset(135,height)
						else choices.Size=UDim2.new(1,-20,0,height) end
						for j,value in values do
							local choice=button(choices,cleanText(value),0,(j-1)*choiceHeight,modern and 104 or compact and 135 or 180,function()
								option:SetValue(value,true); open=false; tween:Tween(row,uiMotion,{Size=UDim2.new(1,0,0,baseHeight)})
							end)
							choice.Size=UDim2.new(1,-3,0,choiceHeight)
						end
						tween:Tween(row,uiMotion,{Size=UDim2.new(1,0,0,baseHeight+height)})
					end
				end)
				if modern then selection.Position=UDim2.new(1,-128,0,8); selection.Size=UDim2.fromOffset(104,24); selection.BackgroundColor3=Color3.fromRGB(30,31,35)
				elseif compact then selection.Position=UDim2.new(1,-145,0,5); selection.Size=UDim2.fromOffset(135,22); selection.BackgroundColor3=Color3.fromRGB(64,68,75)
				else selection.Position=UDim2.fromOffset(10,22); selection.Size=UDim2.new(1,-20,0,36); selection.BackgroundColor3=Color3.fromRGB(45,45,45); addCorner(selection,UDim.new(0,8)); local outline=create('UIStroke',selection,{Color=Color3.fromRGB(64,64,64),Thickness=2,ApplyStrokeMode=Enum.ApplyStrokeMode.Border}) end
				...
				watch(selection,function()
					...
					if chevron:GetAttribute('TargetRotation')~=rotation then chevron:SetAttribute('TargetRotation',rotation); tween:Tween(chevron,uiMotion,{Rotation=rotation}) end
				end)
			elseif kind == 'TextBox' then
				...
				if modern then text.Size=UDim2.fromOffset(220,28); text.BackgroundColor3=Color3.fromRGB(30,31,35)
	...
	for index,name in categoryNames do
		...
		header.MouseButton2Click:Connect(function() panel.Expanded=not panel.Expanded; if panel.Resize then panel.Resize() end end)
	end
	...
	addCorner(modernShell,UDim.new(0,20)); addDragHandler(modernShell)
	...
	modernNav.MouseEnter:Connect(function() ui.ModernNavExpanded=true; ui:LayoutModern() end)
	modernNav.MouseLeave:Connect(function() ui.ModernNavExpanded=false; ui:LayoutModern() end)
	...
	local function setExpanded(module)
		...
		elseif ui.Mode=='Dropdown' then
			ui.Expanded[module.Name]=not ui.Expanded[module.Name]
			local view=ui.SettingsViews[module.Name]
			if view then view.Resize(); return end
		end
		ui:Render()
	end
	local function moduleRow(parent,module,mode,index)
		...
		if mode=='Modern' then
			...
			name=label(row,cleanText(module.Name),84,8,330,30,20); name.FontFace=uipallet.Font
			local description=label(row,cleanText(module.Tooltip or ''),84,39,480,23,13); description.Name='Description'; description.TextColor3=Color3.fromRGB(128,134,141); description.TextTransparency=0.2
			...
			row.MouseEnter:Connect(function() tween:Tween(description,uiMotion,{TextTransparency=0}) end)
			row.MouseLeave:Connect(function() tween:Tween(description,uiMotion,{TextTransparency=0.2}) end)
		elseif mode=='Compact' then
			name=label(row,cleanText(module.Name),10,0,210,40,15); name.Size=UDim2.new(1,-144,0,40); name.FontFace=uipallet.FontSemiBold
			if module.Bind then
				...
				watch(bindPill,function()
					...
					bindPill.Position=UDim2.new(1,-40-bindPill.Size.X.Offset,0,12)
				end)
			end
		...
		watch(row,function()
			...
			if mode=='Dropdown' then
				local targetColor=module.Enabled and color or (hovered and Color3.fromRGB(44,46,54) or Color3.fromRGB(35,37,43))
				if lastEnabled~=module.Enabled or lastHovered~=hovered then
					lastEnabled,lastHovered=module.Enabled,hovered
					visualRevision+=1
					local revision=visualRevision
					visualTransition=true
					local motion=tween:Tween(row,TweenInfo.new(.3,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{BackgroundColor3=targetColor})
					if motion then motion.Completed:Once(function() if revision==visualRevision then visualTransition=false end end) else visualTransition=false end
				elseif not visualTransition then row.BackgroundColor3=targetColor end
				local rotation=ui.Expanded[module.Name] and 180 or 0
	...
	function ui:LayoutModern()
		...
		tween:Tween(modernNav,uiMotion,{Size=UDim2.fromOffset(navWidth,510)})
		...
		tween:Tween(modernList,uiMotion,{Position=UDim2.fromOffset(contentX,40)})
		local detailOpen=self.SelectedModule~=nil and self.Mode=='Modern'
		tween:Tween(modernDetails,uiMotion,{Position=UDim2.fromOffset(contentX+620,0)})
		tween:Tween(modernShell,uiMotion,{Size=UDim2.fromOffset(740+(self.ModernNavExpanded and 110 or 0)+(detailOpen and 250 or 0),510)})
	end
	...
	function ui:Render()
		...
		for name,panel in self.Panels do
			...
			if self.Mode=='Dropdown' then
				...
				for index,module in getModules(name) do
					...
					local settings=create('Frame',panel.List,{Name='Settings_'..module.Name,BackgroundColor3=Color3.fromRGB(32,32,32),Size=UDim2.new(1,0,0,0),ClipsDescendants=true,LayoutOrder=index*2+1})
					local content=create('Frame',settings,{BackgroundTransparency=1,Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y})
					local controls=self:DrawControls(content,module,true,'Dropdown')
					local function resize()
						local h=self.Expanded[module.Name] and controls.AbsoluteContentSize.Y/math.max(scale.Scale,0.01) or 0
						tween:Tween(settings,TweenInfo.new(.25,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,0,h)})
					end
					self.SettingsViews[module.Name]={Resize=resize}
					controls:GetPropertyChangedSignal('AbsoluteContentSize'):Connect(resize)
					resize()
				end
		...
		elseif self.Mode=='Compact' then
			...
			for index,module in getModules(self.Selected) do
				...
				local estimated=40+math.max(0,layout.AbsoluteContentSize.Y/math.max(scale.Scale,0.01))+20
	...
	clickModule:CreateToggle({Name='Transparent',Default=false,Function=function(value)
	...
	local function updateHUD()
		...
		for index,name in names do
			...
			if not entry then
				...
				local shadow=label(row,textValue,6,4,width-8,20,20); shadow.TextXAlignment=Enum.TextXAlignment.Right; shadow.TextColor3=Color3.new(); shadow.TextTransparency=.15
				local text=label(row,textValue,5,2,width-8,20,20); text.TextXAlignment=Enum.TextXAlignment.Right
				entry={Object=row,Text=text,Shadow=shadow,Top=top,Revision=0}; arrayRows[name]=entry
			end
			local target=UDim2.new(1,0,0,(index-1)*24+offset)
			if entry.Target~=target or entry.Leaving then
				entry.Revision+=1; entry.Leaving=false; entry.Target=target
				if animated then tween:Tween(entry.Object,TweenInfo.new(.25,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Position=target}) else tween:Cancel(entry.Object); entry.Object.Position=target end
			end
			entry.Object.Size=UDim2.fromOffset(width,24)
			entry.Text.Text=textValue; entry.Shadow.Text=textValue
			entry.Text.Size=UDim2.fromOffset(width-8,20); entry.Shadow.Size=entry.Text.Size
			entry.Shadow.Visible=hudShadow.Enabled; entry.Top.Visible=index==1
			entry.Text.TextColor3=tenacity:GetThemeColor((index-1)*.20)
		end
		for name,entry in arrayRows do
			if not alive[name] and not entry.Leaving then
				entry.Leaving=true; entry.Revision+=1
				local revision=entry.Revision
				local function remove()
					if entry.Leaving and entry.Revision==revision then entry.Object:Destroy(); arrayRows[name]=nil end
				end
				if animated then
					local motion=tween:Tween(entry.Object,uiMotion,{Position=UDim2.new(1,entry.Object.Size.X.Offset+12,0,entry.Object.Position.Y.Offset)})
					if motion then motion.Completed:Once(remove) else remove() end
				else remove() end
			end
		end
	end
